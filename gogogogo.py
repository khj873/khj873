        ...
        if len(account)>=2:
            ...
            for ac in account:
                ...
                if ac['currency']!='KRW':
                    ...
                    for dd in dddd:
                        if dd['side']=='bid':
                            if not ticker in 매수딕셔너리: #매수딕셔너리ticker가 아예 없다면
                                매수리스트=[]
                                매수리스트.append([dd['uuid'], float(dd['price']), 0, dd['volume'], False, False]) #매수아이디, 매수가격, 매도가격, 매수량, 매도걸었는지 판단, 진행기록
                                매수딕셔너리[ticker]=매수리스트
                                #매수딕셔너리 저장
                                with open('dictsave.p', 'wb') as file:
                                    pickle.dump( 매수딕셔너리,file)
                    ...
                    uuid=''
                    for 리스트 in 매수딕셔너리[ticker]:
                        if 리스트[5]: #진행중인 거래를 검색
                            uuid=리스트[0]
                            break

                    if uuid=='': #진행중인 거래가 없을 시 최소가격 거래를 기준으로 삼음

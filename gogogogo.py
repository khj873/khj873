                ...
                if ticker in 매수딕셔너리:
                    for 리스트 in 매수딕셔너리[ticker]: #매수아이디, 매수가격, 매수금액, 매수량, 매도걸었는지 판단, 매수수수료
                        if dddd[0]['ord_type']=='market': #시장가라면 매도 시 즉시 빼기
                            break
                        elif float(리스트[1])==float(dddd[0]['price']) and float(리스트[3])== float(dddd[0]['volume']): #([dddd[0]['uuid'], dddd[0]['price'], 6000, dddd[0]['volume'], False, float(dddd[0]['price'])*0.9995])
                    ...
                    for dd in dddd:
                        if dd['side']=='bid':
                            if not ticker in 매수딕셔너리: #매수딕셔너리ticker가 아예 없다면
                                매수리스트=[]
                                매수리스트.append([dd['uuid'], float(dd['price']), 사용가능금액/15, dd['volume'], False, float(dd['price'])*0.9995]) #매수아이디, 매수가격, 매수금액, 매수량, 매도걸었는지 판단, 매수수수료
                                매수딕셔너리[ticker]=매수리스트
                                #매수딕셔너리 저장
                                with open('dictsave.p', 'wb') as file:
                                    pickle.dump( 매수딕셔너리,file)
                ...
                if cd[2]['low'] > ma5[2] and cd[1]['low'] < ma5[1] and cd[1]['close'] < cd[1]['open'] and cd[1]['open'] > ma5[1] and cd[0]['close'] > ma5[0] and ma5[4] < ma5[2]:
                    if 사용가능금액/15 > 6000:
                        ...
                        if 매수봉시간[ticker] != cd[0]['time']:
                            upbit.buy_limit_order(ticker, cd[0]['close'], 사용가능금액/15*0.9995/cd[0]['close'])
                            매수봉시간[ticker]=cd[0]['time']
                            print("매수실행", ticker, cd[0]['close'])
                    else:
                        print("사용가능금액부족! 사용가능금액/15:", 사용가능금액/15)

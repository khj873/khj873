    ...
    try:
        ...
        for ticker in tickers: #아직 사지 않은 티커체크하는 곳
            ...
            if dddd[0]['side']=='ask': #매도완료
                if ticker in 매수딕셔너리:
                    for 리스트 in 매수딕셔너리[ticker]: #매수아이디, 매수가격, 매수금액, 매수량, 매도걸었는지 판단, 매수수수료
                        if float(리스트[1])==float(dddd[0]['price']) and float(리스트[3])== float(dddd[0]['volume']): #([dddd[0]['uuid'], dddd[0]['price'], 6000, dddd[0]['volume'], False, float(dddd[0]['price'])*0.9995])
                            ...
                            if len(매수딕셔너리[ticker])>=1:
                                ...
                                for 리스트 in 매수딕셔너리[ticker]:#최소가격 해당 uid찾기
                                    if 리스트[0]==uuid:
                                        리스트[1] = float(리스트[1]) - float(리스트[5]) - (float(dddd[0]['price'])*0.9995)
                                        break
                            with open('dictsave.p', 'wb') as file:
                                pickle.dump( 매수딕셔너리,file)
                            print("매수딕셔너리 저장함", 매수딕셔너리)
                            break
        ...
        if len(account)>=2:
            ...
            for ac in account:
                ...
                if ac['currency']!='KRW':
                    ...
                    볼륨합계=0
                    for dd in dddd:
                        if float(볼륨합계)==(float(ac['balance'])+float(ac['locked'])):
                            break
                        if dd['side']=='bid':
                            볼륨합계+=float(dd['volume'])
                            if not ticker in 매수딕셔너리: #매수딕셔너리ticker가 아예 없다면
                                매수리스트=[]
                                매수리스트.append([dd['uuid'], float(dd['price']), 6000, dd['volume'], False, float(dd['price'])*0.9995]) #매수아이디, 매수가격, 매수금액, 매수량, 매도걸었는지 판단, 매수수수료
                                매수딕셔너리[ticker]=매수리스트
                                #매수딕셔너리 저장
                                with open('dictsave.p', 'wb') as file:
                                    pickle.dump( 매수딕셔너리,file)
                                print("매수딕셔너리 저장함", ticker, 매수딕셔너리)
                                print(매수딕셔너리[ticker])
                            else:
                                같은게있는가=False
                            ...
                            for i in range(1000):
                                if cd[0]['close'] < 최소이득가:
                                    if cd[0]['close'] + (매도차이*i) > 최소이득가:
                                        최소이득호가 = cd[0]['close'] + (매도차이*(i-1))
                                        break
                                else:
                                    if cd[0]['close'] + (매도차이*i) < 최소이득가:
                                        최소이득호가 = cd[0]['close'] - (매도차이*(i+1))
                                        break
                            if 최고점[ticker] > 최소이득호가 and 최소이득호가!=0:
                                #매도걸은 거래인지 확인해야함
                                ...
                                if cd[0]['close'] < 최소이득호가:
                                    ...
                                    if len(orderNumList)>0:
                                        for orderNum in orderNumList:
                                            upbit.cancel_order(orderNum['uuid'])
                                    #시장가매도 실행
                                    upbit.sell_market_order(ticker, float(리스트[3]))
                                    print("시장가매도", ticker, float(리스트[3]))
                                ...
                                if 리스트[4] == False:
                                    #손해매도
                                    if cd[0]['low'] == 최소이득호가:
                                        upbit.sell_limit_order(ticker, 최소이득호가, float(리스트[3]))
                                        리스트[4] = True
                                    ...
                                    if cd[0]['close'] > 최소이득호가:
                                        ...
                                        if cd[거래량최고봉]['low'] > cd[0]['close']:
                                            upbit.sell_limit_order(ticker, cd[0]['close'], float(리스트[3]))
                                            리스트[4] = True
                ...
                if cd[0]['low'] > 매수선 and 매수선!=0:
                    ...
                    if 사용가능금액 > 6000:
                        ...
                        if 매수봉시간[ticker] != cd[0]['time']:
                            upbit.buy_limit_order(ticker, cd[0]['close'], 6000*0.9995/cd[0]['close'])

            ...
            if dddd[0]['side']=='ask': #매도완료
                if ticker in 매수딕셔너리:
                    for 리스트 in 매수딕셔너리[ticker]: #매수아이디, 매수가격, 매수금액, 매수량, 매도걸었는지 판단, 매수수수료
                        if dddd[0]['ord_type']=='market': #시장가라면 매도 시 즉시 빼기
                            break
                        elif 리스트[4]==True: #([dddd[0]['uuid'], dddd[0]['price'], 6000, dddd[0]['volume'], False, float(dddd[0]['price'])*0.9995])
                            ...
                            if len(매수딕셔너리[ticker])>=1:
                                ...
                                for 리스트 in 매수딕셔너리[ticker]:#최소가격 해당 uid찾기
                                    if 리스트[0]==uuid:
                                        print(리스트)
                                        리스트[1] = float(리스트[1]) - (float(리스트[1]) - float(리스트[5])) - (float(리스트[1]) - (float(dddd[0]['price'])*0.9995))
                                        print(리스트)
                                        break
                    ...
                    for dd in dddd:
                        if dd['side']=='bid':
                            if not ticker in 매수딕셔너리: #매수딕셔너리ticker가 아예 없다면
                                매수리스트=[]
                                매수리스트.append([dd['uuid'], float(dd['price']), 사용가능금액/나누는수, dd['volume'], False, float(dd['price'])*0.9995]) #매수아이디, 매수가격, 매수금액, 매수량, 매도걸었는지 판단, 매수수수료
                                매수딕셔너리[ticker]=매수리스트
                                #매수딕셔너리 저장
                                with open('dictsave.p', 'wb') as file:
                                    pickle.dump( 매수딕셔너리,file)
                            ...
                            for i in range(1000):
                                if cd[0]['close'] < 이득도달선:
                                    ...
                                    if cd[0]['close'] - (매도차이*i) < 이득도달선:
                                        ...
                                        break
                            if not 리스트[4]:
                                print( ticker,"최소이득선", 최소이득선,"최소이득호가", 최소이득호가, "최고점",최고점[ticker], "매수가격",매수가격)
                                ...
                                if 리스트[4] == False:
                                    ...
                                    if cd[0]['close'] > 최소이득호가:
                                        ...
                                        if cd[거래량최고봉]['low'] > cd[0]['close']:
                                            ...
                                            if float(ac['balance'])<float(리스트[3]):
                                                upbit.sell_limit_order(ticker, cd[0]['close'], float(ac['balance']))
                                            else:

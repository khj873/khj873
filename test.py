                        ...
                        if cd[0]['close']<cd[1]['low']:
                            if cd[0]['close']>float(ac['avg_buy_price']):
                                ...
                                if bal > 0:
                                    ...
                                    print("매도-이익")
                        if cd[0]['close']<(bb[i]['BH']-bb[i]['BM'])/2+bb[i]['BM']:
                            #매도
                            bal = upbit.get_balance(ticker)
                            if bal > 0:
                                upbit.sell_market_order(ticker, bal)
                            ...
                            if cd[i]['high']>bb[i]['BM'] and cd[i]['open']>cd[i]['close'] and cd[i]['low']<bb[i]['BM']:
                                if cd[i]['low']>cd[bp]['low'] and cd[i]['open']>cd[bp]['open']:
                                    if cd[i]['low']<(bb[i]['BM']-bb[i]['BL'])/2+bb[i]['BL']:
                                        ...
                                        tp=i
                                        break
                        if sell==False and tp!=0:
                            for i in range(tp):
                                if (bb[i]['BH']-bb[i]['BM'])*2/3+bb[i]['BM']<=cd[0]['close']:
                                    sell=True
                                    #절반매도
                                    bal = upbit.get_balance(ticker)
                                    if bal > 0:
                                    ...
                                    break
                    if sell==True:
                        if cd[0]['open']<bb[0]['BM'] and cd[0]['open']>cd[0]['close']:
                            #매도
                            bal = upbit.get_balance(ticker)
                            if bal > 0:
                                upbit.sell_market_order(ticker, bal)
                                print(pyupbit.get_current_price(ticker))
                                print("매도-손해22")
                    if cd[0]['open']<bb[0]['BL'] and cd[0]['open']>cd[0]['close']:
                        #매도
                        bal = upbit.get_balance(ticker)
                        if bal > 0:
                            upbit.sell_market_order(ticker, bal)

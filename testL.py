...
def get_rsi(target_item, tick_kind, inq_range):
    try:
        ...
        rsi1 = round(rsi(df, 14).iloc[-1], 4)
        rsi2 = round(rsi(df, 14).iloc[-2], 4)
        rsi3 = round(rsi(df, 14).iloc[-3], 4)
        ...
        if len(account)>=2:
            ...
            for ac in account:
                if ac['currency']!='KRW':
                    ...
                    rsi, rsi2, rsi3=get_rsi(ticker, '3', '200')
                    if rsi2<69:
                        #매도
            ...
            for ticker in tickers:
                cd, bb=get_bb(ticker, '3', '200', 30)
                rsi, rsi2, rsi3=get_rsi(ticker, '3', '200')
                #print(ticker, rsi,rsi2, rsi3)
                if rsi2>=70 and rsi3>=70:
                    #매수.

...
for t in tempT:
    try:
        가격=pyupbit.get_current_price(t)
        if 가격>1200 and 가격<300000:
            tickers.append(t)
        ...
        if len(account)>=2:
            ...
            for ac in account:
                if ac['currency']!='KRW':
                    ...
                    cd, bb, ma15=get_bb50(ticker, '3', '200', 75)

                    if buyCount<=0:
                        if cd[0]['close']>=(bb[0]['BH']-bb[0]['BM'])/3+bb[0]['BH']:
                            krw=upbit.get_balance("KRW")
                            if krw/2 > 5000:
                                upbit.buy_market_order(ticker, krw*0.9995/2)
                                buyCount=1
                                sellCount-=1
                                print("1차 추가 매수:", ticker)
                    if buyCount==1:
                        if cd[0]['close']>=(bb[0]['BH']-bb[0]['BM'])*2/3+bb[0]['BH'] and cd[0]['close']<bb[0]['BH']:
                            krw=upbit.get_balance("KRW")
                            if krw > 5000:
                                upbit.buy_market_order(ticker, krw*0.9995)
                                buyCount=2
                                sellCount-=1
                                print("2차 총 매수:", ticker)

                    if 절반매도==False and cd[0]['close']>=bb[0]['BH'] and cd[0]['close']/float(ac['avg_buy_price'])>1.001:
                        #절반 이익 매도
                        bal = upbit.get_balance(ticker)
                        if bal > 0:
                            upbit.sell_market_order(ticker, bal/2)
                            절반매도=True
                            print(pyupbit.get_current_price(ticker))
                            이전최고가=cd[0]['high']
                            print("절반 매도")
                    if ma15[0]>ma15[1] and ma15[1]>ma15[2] and ma15[2]>ma15[3] and ma15[0]>ma15[3] and 절반매도==True:
                        if cd[0]['close']<(bb[0]['BH']-bb[0]['BM'])/2+bb[0]['BH']:
                            krw=upbit.get_balance("KRW")
                            if krw > 5000:
                                upbit.buy_market_order(ticker, krw*0.9995)
                                buyCount+=1
                                print("매수 후 여분매수:", ticker)
                    if 절반매도==True and cd[0]['close']>=bb[0]['BH'] and cd[0]['close']/float(ac['avg_buy_price'])>1.001:
                        #재 매수 후 완전 매도
                        if 이전최고가 < cd[0]['close']:
                            if not 완전매도시작:
                                완전매도수 = upbit.get_balance(ticker)
                            if 완전매도수 > 0:
                                upbit.sell_market_order(ticker, 완전매도수/10)
                                이전최고가 = cd[0]['close']
                                완전매도시작=True
                                print(pyupbit.get_current_price(ticker))
                                print("완전매도까지 계속 매도")
                    ...
                    if ma15[0]<ma15[1] and ma15[1]<ma15[2] and ma15[2]<ma15[3] and ma15[0]<ma15[3]:
                        if sellCount<=0:
                            if cd[0]['close']<=(bb[0]['BM']-bb[0]['BL'])*2/3+bb[0]['BL']:
                                #1차 매도
                                bal = upbit.get_balance(ticker)
                                첫매도수 = bal
                                if 첫매도수 > 0:
                                    upbit.sell_market_order(ticker, 첫매도수/3)
                                    buyCount-=1
                                    sellCount=1
                                    print(pyupbit.get_current_price(ticker))
                                    print("1차 매도")
                        if sellCount==1:
                            if cd[0]['close']<=(bb[0]['BM']-bb[0]['BL'])/3+bb[0]['BL']:
                                #2차 매도
                                if 첫매도수 > 0:
                                    upbit.sell_market_order(ticker, 첫매도수/3)
                                    buyCount-=1
            ...
            for ticker in tickers:
                ...
                for i in range(2, len(cd)):
                    ...
                    if cd[i]['low']<bb[i]['BL']:
                        ...
                        break
                if gobuy:
                    if bb[1]['BM']>cd[1]['low'] and bb[1]['BM']<cd[1]['high'] and cd[0]['open'] < cd[0]['close']: #볼린저 밴드 중단을 돌파
                        if ma15[0]>ma15[1] and ma15[1]>ma15[2] and ma15[2]>ma15[3] and ma15[0]>ma15[3]: #15평선이 상승세
                            if bb[0]['BH']/bb[0]['BL']>=1.024:
                                #매수. 매도는 볼린저밴드 상단에서, 손절은 볼린저밴드 중단에서
                                krw=upbit.get_balance("KRW")
                                총액=krw*0.9995
                                if krw/3 > 5000:
                                    upbit.buy_market_order(ticker, 총액/3)
                                    sellCount=0
                                    buyCount=0
                                    첫매도수=0
                                    절반매도=False

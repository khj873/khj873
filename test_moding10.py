            ...
            for ac in account:
                ...
                if ac['currency']!='KRW':
                    ...
                    if not ticker in 매도하한락:
                        매도하한락[ticker]=bb[0]['BL']
                        for i in range(0, 2000):
                            if cd[0]['high']>매도하한락[ticker]:
                                if 매도하한락[ticker]>=cd[0]['high']-(매도차이*i):
                                    매도하한락[ticker]=cd[0]['high']-(매도차이*i)
                                    break
                            else:
                                if 매도하한락[ticker]<=cd[0]['high']+(매도차이*i):
                                    매도하한락[ticker]=cd[0]['high']+(매도차이*i)
                    ...
                    if not ticker in 최고점:
                        최고점[ticker]=0
                    '''else:
                        if 최고점[ticker]<cd[0]['high'] and 수익률>=0:
                            매도하한차이=최고점[ticker]-매도하한락[ticker]
                            최고점[ticker]=cd[0]['high']
                            매도하한락[ticker]=최고점[ticker]-매도하한차이
                    '''
        ...
        for ticker in tickers:
            ...
            if 주문티커[ticker]=="":
                ...
                if cd[2]['low']<bb[2]['BL'] and cd[0]['low']>cd[2]['low'] and cd[1]['open']<cd[1]['close'] and cd[1]['open']>bb[1]['BL'] and cd[1]['close']<bb[1]['BM'] and float(ma5[2])>bb[2]['BL']:
                    ...
                    if len(매수티커리스트)<지갑수:
                        if 사용가능금액 > 5400:
                            upbit.buy_market_order(ticker, 5400)
                            매수봉시간[ticker]=cd[0]['time']

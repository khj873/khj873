            ...
            for ac in account:
                ...
                if ac['currency']!='KRW':
                    ...
                    if not ticker in 중간값:
                        중간값[ticker]=cd[1]['open'] + (한시간예정고점[ticker]-cd[1]['open'])/2
                    ...
                    '''
                    if 예상고점돌파[ticker] and cd[0]['close']<=매도하한락[ticker]:
                        #팔기
                    ...
                    if 중간값[ticker]<=cd[0]['close'] and not 중간매수[ticker]:
                        if 임시총액>5000:
                            if 임시총액/2 > 5000:
                                orderNumList=upbit.get_order(ticker)
                                if len(orderNumList)>0:
                    ...
                    if 중간매수[ticker]:
                        중간매수봉=0
                        for i in range(len(bb)):
                            if 중간값[ticker] <= cd[i]['high']:
                                중간매수봉=i
                                break
                        for i in range(중간매수봉):
                            if cd[i]['low'] <= bb[i]['BL']:
                                break
                            if cd[i]['low'] <= (한시간예정고점[ticker] - ((한시간예정고점[ticker] - 중간값[ticker])*2)):
                                if 임시총액 > 5000:
                                    orderNumList=upbit.get_order(ticker)
                                    if len(orderNumList)>0:
                                        for orderNum in orderNumList:
                                    ...
                                    if bal > 0:
                                        ...
                                        주문티커[ticker]=""
                                        중간매수[ticker]=True
                                        print("시장가추가매수4:", ticker)
                                break



                    '''
                    if 수익률>=0.15*체크횟수[ticker]: #중간 추가매수 기능
                        ...
                        for j in range(1, len(bb)):
                            ...
                            if cd[j]['low']<bb[j]['BL'] and 진입:
                                if low > cd[j]['low']:
                                    low=cd[j]['low']
                        손절점[ticker]=low
                    if (cd[1]['high']+cd[0]['low'])/2 <손절점[ticker]:
                        print(cd[0]['close'], 손절점[ticker])
                        print("진입")
                        #전체 매도
                        #모든 예약 취소하고
                        orderNumList=upbit.get_order(ticker)
                ...
                if cd[2]['low']<bb[2]['BL'] and cd[0]['low']>cd[2]['low'] and cd[1]['open']<cd[1]['close'] and cd[1]['open']>bb[1]['BL'] and cd[1]['close']<bb[1]['BM'] and float(ma5[2])>bb[2]['BL']:
                    ...
                    if len(매수티커리스트)<지갑수:
                        if 사용가능금액 > 6000:
                            ...
                            upbit.buy_limit_order(ticker, cd[1]['open'], 6000*0.9995/cd[1]['open'])
                            t_now = datetime.now()
                            매수주문시간[ticker]=t_now
                            주문티커[ticker]=ticker
                            매수하한점=cd[2]['low']
                            print("시장가매수:", ticker, cd[1]['open'])
                            #print("손절가", 손절점)
                            손절점[ticker]=low
                            한시간예정고점[ticker]=예정고점

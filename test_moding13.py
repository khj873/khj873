    ...
    try:
            ...
            for ac in account:
                ...
                if ac['currency']!='KRW':
                    ...
                    if ticker in 임시매수리스트:
                        임시매수리스트.remove(ticker)
                    if not ticker in 중간매수:
                        if (float(ac['avg_buy_price']) * (float(ac['balance'])+float(ac['locked'])))>=6500:
                            중간매수[ticker]=True
                        else:
                            중간매수[ticker]=False
                    ...
                    if 중간값[ticker]<=cd[0]['high'] and not 중간매수[ticker]:
                        if 임시총액>5000:
                            '''
                            if 임시총액/2 > 5000:
                                orderNumList=upbit.get_order(ticker)
                        ...
                        if len(orderNumList)<=0:
                            ...
                            if 차이간격>0:
                                ...
                                if bal > 0:
                                    ...
                                    upbit.sell_limit_order(ticker, 매도가격, bal)
                    if 중간매수[ticker]:
                        if 중간값[ticker] > cd[0]['high']:
                            매수선=0
                            for i in range(1, len(bb)):
                                if cd[i]['high'] > 중간값[ticker]:
                                    break
                                    ...
                                    for j in range(i+1, len(bb)): #음봉 무리 중에 가장 큰 거래량 찾기
                                        if cd[j]['open']<cd[j]['close']: #양봉이면 검색 종료
                                            ...
                                            if 거래량<cd[j]['vol']:
                                                ...
                                                break
                                    break
                            if cd[0]['close'] > 매수선 and 매수선!=0:
                                #매도한 만큼 매수 후, 매도대기 전부취소
                                if 임시총액 > 5000:
                                    orderNumList=upbit.get_order(ticker)
                                    if len(orderNumList)>0:
                                        for orderNum in orderNumList:
                    ...
                    if 중간매수[ticker]:
                        if 손절점[ticker] < 중간값[ticker] - (한시간예정고점[ticker] - 중간값[ticker]):
                            손절점[ticker] = 중간값[ticker] - (한시간예정고점[ticker] - 중간값[ticker])
        ...
        for ticker in tickers:
            ...
            if 주문티커[ticker]=="":
                ...
                #손절점찾기
                low=999999999
                for i in range(매수제한봉):
                    if cd[i]['low'] < low:
                        low=cd[i]['low']
                
                매수제한선=(예정고점+low)/2
                #print(cd[0]['close'] ,매수선,  매수제한선, ticker)
                if 매수선!=0 and cd[0]['close']>매수선 and cd[0]['close'] < 매수제한선:
                    #매수선 가격에서 매수진입
                    krw=0
                    ...
                    if len(매수티커리스트)+len(임시매수리스트)<지갑수:
                        if 사용가능금액 > 6000:
                            ...
                            upbit.buy_limit_order(ticker, 매수선, 6000*0.9995/매수선)
                            t_now = datetime.now()
                            매수주문시간[ticker]=t_now
                            주문티커[ticker]=ticker
                            매수하한점=cd[2]['low']

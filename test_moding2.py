        ...
        if len(account)>=2:
            ...
            for ac in account:
                if ac['currency']!='KRW':
                    ...
                    cd, bb, ma15=get_bb50(ticker, '5', '200', 75)
            ...
            for ticker in tickers:
                ...
                for i in range(1, len(cd)):
                    if (bb[i]['BM']-bb[i]['BL'])/2+bb[i]['BL']>cd[i]['low']:
                        break
                    if bb[i]['BH']>cd[i]['high']:
                        if downIndex==0:
                            downIndex=i
                    if downIndex!=0:
                        if bb[i]['BH']<cd[i]['high']:
                            if cd[i]['high']>highPrice:
                                highIndex=i
                                highPrice=cd[i]['high']
                if highIndex!=0:
                    if cd[0]['close'] > highPrice and bb[1]['BH']<=highPrice:

    ...
    try:
        ...
        if len(account)>=2:
            ...
            for ticker in tickers:
                ...
                cd, bb, ma15=get_bb50(ticker, '1', '200', 100)
                gobuy=False
                if (cd[1]['low']<bb[1]['BL'] and cd[1]['close']>bb[1]['BM'] and cd[1]['open']<cd[1]['close']) or (cd[2]['low']<bb[2]['BL'] and cd[1]['low']>bb[1]['BL'] and cd[1]['close']>bb[1]['BM'] and cd[1]['open']<cd[1]['close']):
                    
                    매수계산가=(cd[1]['high']-cd[1]['low'])/2+cd[1]['low']
                    orderbookList=pyupbit.get_orderbook(ticker)
                    매수가=0
                    for hoga in orderbookList['orderbook_units']:
                        if float(hoga['bid_price'])<=매수계산가:
                            매수가=float(hoga['bid_price'])
                            break
                    ...
                    if 총액 > 5000:
                        ...
                        print(손절점)
                        break
                if cd[2]['low']<bb[2]['BL'] and cd[0]['low']>cd[2]['low'] and cd[1]['open']<cd[1]['close'] and cd[1]['low']>bb[1]['BL'] and cd[0]['close']<bb[0]['BM']:
                    매수가=cd[1]['open']
                    최종매수가격[ticker]=cd[1]['open']

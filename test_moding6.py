...
tempT2=[]
if int(len(tickers)*2/3)==0:
    for i in range(1):
        tempT2.append(tickers[i])
else:
    for i in range(int(len(tickers)*2/3)):
        tempT2.append(tickers[i])
tickers.clear()
tickers=tempT2
print(tickers)
            ...
            for ac in account:
                ...
                if ac['currency']!='KRW':
                    ...
                    if float(ac['balance'])>0 and 주문티커=="":
                        print("매도대기 평균매수가", float(ac['avg_buy_price']))
                        for i in range(1, 150):
                            매도가=cd[0]['close']+(매도차이*i)
                            print(i, 매도가 / float(ac['avg_buy_price']))
                            if 매도가 / float(ac['avg_buy_price']) > 1.003:
                                if 매도가 / float(ac['avg_buy_price']) >1.006:
                                    break
                                else:
                                    print("가격", cd[0]['close'])
                                    print("매도가", 매도가)
                        ...
                        if cd[1]['close']<cd[0]['open']:
                            ...
                            if 총액/16 > 5000:
                                if percent < 2:
                                    ...
                                    upbit.buy_limit_order(ticker, 매수가, 매수개수/8)
                                매수봉시간=cd[0]['time']
                                t_now = datetime.now()
                                매수주문시간=t_now
                                주문티커=ticker
                            ...
                            if 총액/16 > 5000:
                                if percent < 2:
                                    ...
                                    upbit.buy_market_order(ticker, 총액/8)
                                매수봉시간=cd[0]['time']
                                #upbit.buy_limit_order(ticker, 매수가, 매수개수/16)
                                t_now = datetime.now()
                                매수주문시간=t_now

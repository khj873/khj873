            ...
            for ac in account:
                ...
                if ac['currency']!='KRW':
                    ...
                    if 수익률>0 and 매도하한락[ticker]==0 and 사용가능금액/2 < 매수한총액:
                        if ((최고점[ticker]-(매도차이*2))-float(ac['avg_buy_price']))/float(ac['avg_buy_price'])*100 > 0:
                            매도하한락[ticker]=최고점[ticker]-(매도차이*2)
                    if 수익률>0 and (사용가능금액*9)/10 < 매수한총액 and 최종체크[ticker]==False:
                        if ((최고점[ticker]-(매도차이*2))-float(ac['avg_buy_price']))/float(ac['avg_buy_price'])*100 > 0:
                            매도하한락[ticker]=최고점[ticker]-(매도차이*2)
                            최종체크[ticker]=True
                    ...
                    if 수익률>=0.15*체크횟수[ticker]:
                        if 임시총액>5000:
                            ...
                            if cd[0]['time'] != 매수봉시간[ticker]:
                                print("1", 매도체크리스트차이[ticker])
                                print("1", cd[0]['timestamp']-매도체크리스트[ticker])
                            ...
                            if not ticker in 매수봉시간:
                                매수봉시간[ticker]=0
                            if cd[0]['time'] != 매수봉시간[ticker]:
                                print("2", 매도체크리스트차이[ticker])
                                print("2", cd[0]['timestamp']-매도체크리스트[ticker])

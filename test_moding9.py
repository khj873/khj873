...
def get_volumeOrder(tickerList):
    ...
    for r in responseDict:
        if r['acc_trade_price_24h']>100000000000:
        ...
        if len(account)>=2:
            ...
            for ac in account:
                ...
                if ac['currency']!='KRW':
                    ...
                    krw=사용가능금액-매수한총액
                    임시총액=krw*0.9995
                    if not ticker in 체크횟수:
                        체크횟수[ticker]=1
                    if not ticker in 매도체크리스트차이:
                        매도체크리스트차이[ticker]=99999999999
                    if not ticker in 매도체크리스트:
                        for i in range(1, len(bb)):
                            if cd[i]['low'] < bb[i]['BL']:
                                매도체크리스트[ticker]=cd[i]['timestamp']
                                break
                    ...
                    if 수익률>0 and 매도하한락[ticker]==0 and 사용가능금액/2 < 매수한총액:
                        if ((최고점[ticker]-(매도차이*3))-float(ac['avg_buy_price']))/float(ac['avg_buy_price'])*100 > 0:
                            매도하한락[ticker]=최고점[ticker]-(매도차이*3)
                    if 수익률>0 and (사용가능금액*2)/3 < 매수한총액 and 최종체크[ticker]==False:
                        if ((최고점[ticker]-(매도차이*3))-float(ac['avg_buy_price']))/float(ac['avg_buy_price'])*100 > 0:
                            매도하한락[ticker]=최고점[ticker]-(매도차이*3)
                            최종체크[ticker]=True

                    if cd[0]['close']<=매도하한락[ticker]:
                        #팔기

                        #예약가격이 자신보다 높은 모든 예약 취소하고
                        orderNumList=upbit.get_order(ticker)
                        if len(orderNumList)>0:
                            for orderNum in orderNumList:
                                if float(orderNum['price']) > 매도하한락[ticker]+매도차이:
                                    upbit.cancel_order(orderNum['uuid'])
                        ...
                        if float(ac['balance'])>0:
                            ...
                            if bal > 0:
                                upbit.sell_limit_order(ticker, 매도가+매도차이, bal/2)
                                print(매도차이)
                                print("매도가ccc",매도가+매도차이)
                                티켓별매도가격[ticker]=매도가+매도차이
                    ...
                    if 수익률>=0.15*체크횟수[ticker]:
                        if 임시총액>5000:
                            ...
                            if cd[0]['time'] != 매수봉시간[ticker]:
                                print("1", cd[0]['timestamp']-매도체크리스트[ticker],매도체크리스트차이[ticker])
                                print("체크횟수", 체크횟수[ticker], ticker)
                                if cd[0]['timestamp']-매도체크리스트[ticker] <= 매도체크리스트차이[ticker]: #작으면 사고 크면 판다
                                    매도체크리스트차이[ticker]=cd[0]['timestamp']-매도체크리스트[ticker]
                                    매도체크리스트[ticker]=cd[0]['timestamp']
                                    if 임시총액/4 > 5000:
                                        매수개수=(임시총액/4)/매수가
                                        orderNumList=upbit.get_order(ticker)
                                        if len(orderNumList)>0:
                                    ...
                                    if float(ac['balance'])>0:
                                        매도가=cd[0]['close']
                                        print("가격aaa", cd[0]['close'])
                                        print("매도가aaa", 매도가)
                                        print("avgaaa", float(ac['avg_buy_price']))
                                        bal = upbit.get_balance(ticker)
                                        if bal > 0:
                                            upbit.sell_limit_order(ticker, 매도가, bal)
                                            print(매도차이)
                            ...
                            if not ticker in 매수봉시간:
                                매수봉시간[ticker]=0
                            if cd[0]['time'] != 매수봉시간[ticker]:
                                print("2", cd[0]['timestamp']-매도체크리스트[ticker],매도체크리스트차이[ticker])
                                print("2체크횟수", 체크횟수[ticker], ticker)
                                if cd[0]['timestamp']-매도체크리스트[ticker] > 매도체크리스트차이[ticker]: #크면 판다
                                    매도체크리스트차이[ticker]=cd[0]['timestamp']-매도체크리스트[ticker]
                                    매도체크리스트[ticker]=cd[0]['timestamp']
                                    if float(ac['balance'])>0:
                                        매도가=cd[0]['close']
                                        print("가격bbb", cd[0]['close'])
                                        print("매도가bbb", 매도가)
                                        print("avgbbb", float(ac['avg_buy_price']))
                                        bal = upbit.get_balance(ticker)
                                        if bal > 0:
                                            upbit.sell_limit_order(ticker, 매도가, bal)
                                            print(매도차이)
                ...
                if cd[2]['low']<bb[2]['BL'] and cd[0]['low']>cd[2]['low'] and cd[1]['open']<cd[1]['close'] and cd[1]['open']>bb[1]['BL'] and cd[1]['close']<bb[1]['BM'] and float(ma5[2])>bb[2]['BL']:
                    ...
                    for j in range(1, len(bb)):
                        if 진입==False and cd[j]['low']<bb[j]['BL']:
                            진입=True
                            빠져나온후=0
                            for i in range(j+1, len(bb)):
                                if cd[i]['low']>=bb[i]['BL']:
                                    빠져나온후=i
                                    break
                            j=빠져나온후
                        if 진입 and 빠져나온후!=0 and cd[j]['low']>=bb[j]['BM']:
                            통과=True
                            break
                        if 진입 and 빠져나온후!=0 and cd[j]['low']<bb[j]['BL']:
                            통과=False
                            break
                    if not 통과:
                        break

            ...
            if len(orderNumList)>0:
                for orderNum in orderNumList:
                    if orderNum['side']=='bid':
                        ...
                        if 총액 > 5000:
                            t_now = datetime.now()
                            upbit.buy_market_order(주문티커, 총액)
                            손절점=999999999999

                            cd, bb, ma5=get_bb50(주문티커, '1', '200', 100)
                            매수봉=0
                            ...
                            손절범위=매수가-손절점
                            손절수정=False
                            if float(upbit.get_balances()[1]['avg_buy_price'])>bb[0]['BM']:
                                매수반넘음=True
                            else:
                                매수반넘음=False
                            print("시장가매수")
        ...
        if len(account)>=2:
            ...
            for ac in account:
                if ac['currency']!='KRW':
                    ...
                    '''
                    cd, bb, ma5=get_bb50(ticker, '1', '200', 100)
                    if 매수반넘음:
                        cd, bb, ma5=get_bb50(ticker, '3', '200', 100)
                    '''
                    rsiList=get_rsi(ticker, '1', '200')
                    
                    if (rsiList[1]>70 and rsiList[0]<70) or rsiList[0]<30:
                        #전체 매도
                        #매도예약 취소하고
                        ...
                        if bal > 0:
                            ...
                            #upbit.sell_limit_order(ticker, 손절점, bal)
            ...
            for ticker in tickers:
                ...
                cd, bb, ma5=get_bb50(ticker, '1', '200', 100)
                cd_3, bb_3, ma5_3=get_bb50(ticker, '3', '200', 100)
                rsiList=get_rsi(ticker, '1', '200')
                gobuy=False
                '''
                if rsiList[0]>70 and rsiList[1]<68:
                    for i in range(2, len(bb)):
                        if rsiList[i]>70:
                            gobuy=True
                '''
                if (rsiList[0]>40 and rsiList[1]<30):
                    gobuy=True
